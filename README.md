...
4. Hybrid Search：精确查询会启用 `exact_match + sparse_keyword + dense_vector`。字面命中保证条款名不丢，向量召回保证用户表达不完全一致时还能召回。
5. Scope-aware Rerank：重排阶段会把目标章节、目标条款、目标子条款和问题面作为强特征；相邻但不属于目标的问题会被降权，不能回答问题面的候选也会降权。Rerank 是业务可选增强，但在制度问答里很推荐。
6. Span Extraction：如果历史数据里仍然存在粗 chunk，系统会在命中的 chunk 内二次截取目标范围。例如从 `（二）二类违规行为` 截到 `（三）三类违规行为` 之前，或从 `1.2二类违规行为` 截到 `1.3三类违规行为` 前，并去掉下一条编号残留。
7. Scope Guard：证据质量检查会记录是否成功收窄范围，并检查结果里是否混入竞争章节。这个检查用于调试和教学，也用于后续接入 LLM 前的安全闸。
8. 参见型片段过滤：如果命中的片段只是“具体参见《某制度》”，它只能说明线索在哪里，不能作为最终答案证据。系统会优先保留直接包含定义、章节或条款正文的 direct evidence。
9. Citation Merge：如果同一篇制度的多个相邻 chunk 都命中，展示层会按文档、章节和范围合并引用，避免重复来源刷屏。
10. 可追溯链接：云谷制度链接使用真实详情地址 `https://work.yungu.org/policyDetail/{importInformationId}`，不会生成未验证的前端路由。
...
- Query understanding：把问题拆成目标对象和问题面，例如目标对象是 `二类违规行为`，问题面是 `处罚/处分`。
