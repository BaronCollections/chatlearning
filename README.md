...
4. 行为级事实映射：有些问题不是章节名，而是业务事实。例如 `我旷工两天会受到什么处罚` 会被识别为 `target_behavior=absenteeism`、`behavior_duration=2天`，再映射到制度里的 `连续旷工3个工作日以下` 规则。回答时会组合互补证据：`员工纪律制度` 说明它属于 `二类违规行为 / 破坏学校管理秩序行为 / 4.2旷工少于三天`，`工作时间及假期管理制度` 说明处理结果是扣除旷工期间工资并给予记过处分。
...
9. 参见型片段过滤：如果命中的片段只是“具体参见《某制度》”，它只能说明线索在哪里，不能作为最终答案证据。系统会优先保留直接包含定义、章节或条款正文的 direct evidence。
10. Citation Merge：如果同一篇制度的多个相邻 chunk 都命中，展示层会按文档、章节和范围合并引用，避免重复来源刷屏。
11. 可追溯链接：云谷制度链接使用真实详情地址 `https://work.yungu.org/policyDetail/{importInformationId}`，不会生成未验证的前端路由。
...
- Query understanding：把问题拆成目标对象、行为条件和问题面，例如目标对象是 `二类违规行为`，或行为条件是 `旷工两天`，问题面是 `处罚/处分`。
